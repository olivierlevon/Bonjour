...
{
	mDNSInterfaceData *i; //NetworkInterfaceInfoOSX
	for (i = mDNSStorage.p->interfaceList; i; i = i->next)
//		if (i->Exists)
		{
			if ((i->interfaceInfo.ip.type == mDNSAddrType_IPv4) &&
				i->interfaceInfo.ip.ip.v4.NotAnInteger == addr->ip.v4.NotAnInteger)
				return i->interfaceInfo.InterfaceID;
			else if ((i->interfaceInfo.ip.type == mDNSAddrType_IPv6) &&
					 (i->interfaceInfo.ip.ip.v6.l[0] == addr->ip.v6.l[0] &&
					  i->interfaceInfo.ip.ip.v6.l[1] == addr->ip.v6.l[1] &&
					  i->interfaceInfo.ip.ip.v6.l[2] == addr->ip.v6.l[2] &&
					  i->interfaceInfo.ip.ip.v6.l[3] == addr->ip.v6.l[3])
					 )
				return i->interfaceInfo.InterfaceID;
		}
	return(NULL);
...
//	SetupActiveInterfaces(utc);
	...
	(void)SetupInterfaceList(&mDNSStorage);
...
//mDNSexport mDNSBool mDNSWindowsCreateEtcHostsEntry_ut(const domainname *domain, const struct sockaddr *sa, const domainname *cname, char *ifname, AuthHash *auth)
{
	return mDNSWin32CreateEtcHostsEntry(&mDNSStorage, domain, sa, cname, ifname, auth);
//	return true;
}
